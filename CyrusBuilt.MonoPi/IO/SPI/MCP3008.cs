...
namespace CyrusBuilt.MonoPi.IO.SPI
{
	/// <summary>
	...
	public class MCP3008 : IDisposable
	{
		...
		public Int32 ReadADC() {
			...
			this._chipSelect.Write(PinState.High);
			this._clock.Write(PinState.Low);       // Drive clock pin low.
			this._chipSelect.Write(PinState.Low);  // Drive chip-select low.
			...
			for (Int32 i = 0; i < 5; i++) {
				if ((cmd & 0x80) == 128) {
					this._masterOutSlaveIn.Write(PinState.High);
				}
				else {
					this._masterOutSlaveIn.Write(PinState.Low);
				}
				cmd <<= 1;
			...
			for (Int32 j = 0; j < 12; j++) {
				...
				this._clock.Write(PinState.Low);
				adcout <<= 1;
				if (this._masterInSlaveOut.Read() == PinState.High) {
					adcout |= 0x1;
				}
			}
